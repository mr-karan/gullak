...
const GULLAK_SECRET = 'CHANGE_ME_to_a_long_random_string';
...
function menuRefreshCalendar() { buildYearlyCalendar(); }
function menuRefreshAnalysis() { calculateMonthlyScore(); buildWeeklyAnalysis(); }
...
function handleEdit(e) {
  ...
  if (name === SHEETS.TRACKER) {
    debounce('cal', function () { repaintEditedMonth(e); });
    return;
  }
  if (name === SHEETS.SETUP) {
    debounce('cal', function () { buildYearlyCalendar(); });
    return;
  }
...
function monthLayout(year) {
  ...
  for (let m = 0; m < 12; m++) {
    ...
    layout.push({ startCol: col, weeks: weeks, startDay: startDay, days: days });
    col += weeks + 1;
  }
...
function ensureCalendarLayout(year) {
  ...
  if (Number(PropertiesService.getDocumentProperties().getProperty('calYear')) === year) {
    return sheet; // layout already built for this year
  }

  sheet.clear();
  sheet.clearNotes();
  sheet.getRange('A1').setValue('Year: ' + year).setFontSize(14).setFontWeight('bold');
  ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
    .forEach(function (d, i) { sheet.getRange(5 + i, 1).setValue(d); });
  ...
  PropertiesService.getDocumentProperties().setProperty('calYear', String(year));
  return sheet;
}
...
function spendMapAndStats(tracker, tz) {
  ...
  if (lastRow > 1) {
    tracker.getRange(2, 1, lastRow - 1, 7).getValues().forEach(function (row) {
      const date = row[0];
  ...
  const spends = Object.values(map).filter(function (v) { return v > 0; });
  const avg = spends.length ? spends.reduce(function (a, b) { return a + b; }, 0) / spends.length : 0;
  return {
    map: map,
    avg: avg,
...
function dayColor(spend, s) {
  ...
  if (spend <= s.avg) {
    return interpolate(COLORS.YELLOW, COLORS.ORANGE, (spend - s.min) / Math.max(s.avg - s.min, 1));
  }
  return interpolate(COLORS.ORANGE, COLORS.RED, (spend - s.avg) / Math.max(s.max - s.avg, 1));
}

function paintMonth(sheet, year, m, mo, s, tz, todayKey) {
  const bg = Array.from({ length: 7 }, function () { return Array(mo.weeks).fill(null); });
  const notes = Array.from({ length: 7 }, function () { return Array(mo.weeks).fill(''); });
  for (let d = 1; d <= mo.days; d++) {
    ...
    notes[dayIdx][weekIdx] = Utilities.formatDate(dateObj, tz, 'dd MMM yyyy') + '\nSpend: ₹' + spend;
  }
  sheet.getRange(5, mo.startCol, 7, mo.weeks).setBackgrounds(bg).setNotes(notes);
}
...
function buildYearlyCalendar() {
  ...
  const ml = monthLayout(year);
  ml.layout.forEach(function (mo, m) { paintMonth(sheet, year, m, mo, s, tz, todayKey); });
...
function calculateMonthlyScore() {
  ...
  calc.getRange('G2:I4').getValues().forEach(function (row) {
...
function ensureScoreFormatting() {
  ...
  const kept = sheet.getConditionalFormatRules().filter(function (r) {
    return !r.getRanges().some(function (x) { return x.getA1Notation() === 'J2'; });
  });
  const mk = function (build) { return build.setRanges([range]).build(); };
  sheet.setConditionalFormatRules(kept.concat([
    mk(SpreadsheetApp.newConditionalFormatRule().whenNumberGreaterThan(8).setBackground('#b7e1cd').setFontColor('#000000')),
    mk(SpreadsheetApp.newConditionalFormatRule().whenNumberBetween(7, 7.99).setBackground('#fff2cc').setFontColor('#000000')),
    mk(SpreadsheetApp.newConditionalFormatRule().whenNumberLessThan(7).setBackground('#f4c7c3').setFontColor('#000000')),
  ]));
...
function buildWeeklyAnalysis() {
  ...
  weekly.getRange('D1:H1').setValues([['Week Start', 'Week End', 'Spend', 'Limit', 'Ratio']]);
  if (weeks.length > 0) {
    ...
    weekly.getRange(2, 8, weeks.length, 1).setBackgrounds(colors).setNumberFormat('0"%"');
  }
...
function handleWeeklyModeUI() {
  ...
  if (mode === 'Quarter') {
    quarterCell.setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(['Q1', 'Q2', 'Q3', 'Q4']).build());
    quarterCell.setBackground('#ffffff').setValue('Q' + (Math.floor(monthIdx / 3) + 1));
    [startCell, endCell].forEach(function (c) { c.clearContent(); c.clearDataValidations(); c.setBackground('#eeeeee'); });
  } else if (mode === 'Custom') {
    const rule = SpreadsheetApp.newDataValidation().requireValueInList(months).build();
    startCell.setDataValidation(rule).setBackground('#ffffff').setValue(months[monthIdx]);
    endCell.setDataValidation(rule).setBackground('#ffffff').setValue(months[monthIdx]);
    quarterCell.clearContent(); quarterCell.clearDataValidations(); quarterCell.setBackground('#eeeeee');
  }

  const setup = ss.getSheetByName(SHEETS.SETUP);
  const payments = ['All'].concat(setup.getRange('D11:D20').getValues().flat().filter(Boolean));
  const paymentCell = sheet.getRange('B6');
  paymentCell.setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(payments).build());
...
function doPost(e) {
  try {
    ...
    if (body.secret !== GULLAK_SECRET) return gullakJson({ error: 'unauthorized' });
    ...
    const setup = ss.getSheetByName(SHEETS.SETUP);
    if (setup) {
      setup.getRange('A11:B19').getValues().forEach(function (r) {
