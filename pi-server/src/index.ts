...
console.log(`chavanni v${config.version} listening on http://${config.host}:${config.port}`);
...
// secured only by CHAVANNI_WHATSAPP_API_KEY; without that key it is open to
// anyone who can reach it, regardless of CHAVANNI_HTTP_API_KEY. Warn whenever the
...
if (!config.whatsappApiKey) {
  console.warn(
    "WARNING: CHAVANNI_WHATSAPP_API_KEY is unset — the WhatsApp webhook is " +
...
// Disabled unless CHAVANNI_SHEETS_SYNC_INTERVAL_MIN > 0 and the web-app URL +
