...
test("deterministically parses Kotak sent UPI SMS without LLM", async () => {
  const result = await parseSms(config, {
    sender: "JX-KOTAKB-S",
    body:
      "Sent Rs.146.00 from Kotak Bank AC X2746 to 7204069332@nyes on 06-05-26.UPI Ref 302069196442. Not you? SMS BLOCK",
  ...
  expect(result.candidate?.payee).toBe("7204069332@nyes");
  expect(result.candidate?.accountHint).toBe("Kotak AC X2746");
  expect(result.candidate?.date).toBe("2026-05-06");
  expect(result.candidate?.bankRef).toBe("302069196442");
