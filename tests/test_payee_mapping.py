...
def test_parse_expense_applies_payment_mapping_when_missing(temp_ledger_path):
    ...
    assert (
        result.data["transaction"]["payment_account"] == "Liabilities:CreditCard:Axis"
    )


def test_parse_expense_keeps_explicit_payment_account(temp_ledger_path):
    _write_payee_mapping(
        temp_ledger_path,
        "; gullak:payee_map Swiggy=Expenses:Food:Delivery|Liabilities:CreditCard:Axis",
    )
