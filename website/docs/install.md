...
Chavanni is an Android app today (iOS is planned). It works fully offline out of
...
**Coming soon.** Chavanni is being prepared for the F-Droid catalog. When it
...
1. Open **[github.com/mr-karan/chavanni/releases](https://github.com/mr-karan/chavanni/releases)**
   on your phone and download the latest `chavanni-*.apk`.
...
Chavanni can turn bank SMS into draft transactions you review. This needs the
...
- **It is opt-in.** The app works fully without it. If you never grant SMS
  access, Chavanni is a manual + receipt-scan + sync tracker and nothing about
  SMS runs.
- **Parsing happens on *your* server, not ours.** When SMS capture is on, bank
  ...
  no Chavanni-operated cloud in this path. If you run no server, there is no SMS
  parsing.
...
- **You can turn it off any time.** Disable SMS capture in the app's settings,
  or revoke the permission in Android Settings → Apps → Chavanni → Permissions.
